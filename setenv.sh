...
if [[ $OS == "cygwin" || $OS == "mingw" ]]; then
    ...
    export BAZEL_HOME="$(getPath "$prefix/c/opt/bazel-4")"
    ...
    export SBT_HOME="$(getPath "$prefix/c/opt/sbt-1.5")"
    ...
    export BAZEL_HOME="$(getPath "/opt/bazel-4")"
    ...
    export MILL_HOME="$(getPath "/opt/mill-0.9")"
    export SBT_HOME="$(getPath "/opt/sbt-1.5")"
