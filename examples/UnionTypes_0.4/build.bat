...
if exist "%__CHECK_FILE%" (
    if %_DEBUG%==1 ( echo %_DEBUG_LABEL% "%_DIFF_CMD%" %_DIFF_OPTS% "%__OUTPUT_FILE%" "%__CHECK_FILE%" 1>&2
...
if exist "%__SOURCES_FILE%" del "%__SOURCES_FILE%" 1>NUL
set __N=0
for /f "delims=" %%f in ('dir /s /b "%_SOURCE_DIR%\test\scala\*.scala" 2^>NUL') do (
    echo %%f >> "%__SOURCES_FILE%"
    set /a __N+=1
)
if %__N%==0 (
...
for /f "usebackq" %%f in (`dir /s /b "%_TEST_CLASSES_DIR%\*JUnitTest.class" 2^>NUL`) do (
    ...
    if not %ERRORLEVEL%==0 (
        echo %_DEBUG_LABEL% Failed to execute test "!__MAIN_CLASS!" 1>&2
        set _EXITCODE=1
        goto :eof
    )
)
