...
if %_DEBUG%==1 (
    echo %_DEBUG_LABEL% Options    : _DOTTY=%_DOTTY% _INSTRUMENTED=%_INSTRUMENTED% _TASTY=%_TASTY% _TIMER=%_TIMER% _VERBOSE=%_VERBOSE% 1>&2
...
if exist "%__TARGET_FILE%" (
    for /f "usebackq" %%i in (`powershell -c "gci -path '%__TARGET_FILE%' | select -last 1 -expandProperty LastWriteTime | Get-Date -uformat %%Y%%m%%d%%H%%M%%S"`) do (
        set __TARGET_TIMESTAMP=%%i
    )
) else ( set __TARGET_TIMESTAMP=00000000000000
)
if exist "%__PATH%" (
    for /f "usebackq" %%i in (`powershell -c "gci -recurse -path '%__PATH%' | sort LastWriteTime | select -last 1 -expandProperty LastWriteTime | Get-Date -uformat %%Y%%m%%d%%H%%M%%S"`) do (
        set __SOURCE_TIMESTAMP=%%i
    )
) else ( set __SOURCE_TIMESTAMP=00000000000000
)
