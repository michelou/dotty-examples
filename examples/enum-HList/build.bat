...
if not errorlevel 0 (
    echo %_ERROR_LABEL% PFailed to execute Scala main class "%_MAIN_CLASS%" 1>&2
    set _EXITCODE=1
    goto :eof
)
if %_TASTY%==1 (
...
if %_DEBUG%==1 ( echo %_DEBUG_LABEL% "%_JAVA_CMD%" -jar "!__JACOCO_CLI_FILE!" report "%__EXEC_FILE%" --classfiles "%_CLASSES_DIR%" --encoding UTF8 --html "%__TARGET_HTML_DIR%" --name "%_PROJECT_NAME%" --quiet --sourcefiles "%_MAIN_SOURCE_DIR%\scala" 1>&2
