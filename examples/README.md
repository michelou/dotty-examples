...
Command [**`build`**](enum-Planet/build.bat) is a basic build tool consisting of ~600 lines of batch/[Powershell ][microsoft_powershell] code <sup id="anchor_01">[[1]](#footnote_01)</sup> featuring subcommands **`clean`**, **`compile`**, **`decompile`**, **`doc`**, **`help`**, **`run`** and **`test`**.
...
Finally, command [**`build -verbose decompile`**](enum-Planet/build.bat) decompiles the generated bytecode:
