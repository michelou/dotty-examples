...
set _JAVAC_OPTS=-classpath %_PROJECT_CPATH%%_CLASSES_DIR% -d %_CLASSES_DIR%
...
set __COMPILE_OPTS=%_COMPILE_OPTS% -classpath "%_PROJECT_CPATH%%_CLASSES_DIR%" -d %_CLASSES_DIR%
...
:libs_cpath
set _LIBS_CPATH=
for /f %%i in ('where dotc.bat') do set __DOTTY_BIN_DIR=%%~dpi
for /f %%f in ("!__DOTTY_BIN_DIR!..") do set __DOTTY_HOME=%%~sf
for /f %%i in ('dir /b "!__DOTTY_HOME!\lib\dotty*.jar"') do (
    set _LIBS_CPATH=!_LIBS_CPATH!!__DOTTY_HOME!\lib\%%i;
)
