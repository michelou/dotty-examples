...
if %__N%==0 (
    echo %_WARNING_LABEL% No TASTy source file found 1>&2
    goto :eof
) else if %__N%==1 ( set __N_FILES=%__N% TASTy source file
) else ( set __N_FILES=%__N% TASTy source files
)
if %_DEBUG%==1 ( echo %_DEBUG_LABEL% "%_SCALAC_CMD%" "@%__OPTS_FILE%" "@%__SOURCES_FILE%" 1>&2
...
if defined __ADD_SCALA3_LIBS (
    ...
    for / "delims=" %%f in ("%SCALA3_HOME%\lib\*.jar") do (
...
if %_SCALA_VERSION%==2 (
    echo -classpath "%__CPATH:\=\\%" -d "%_TARGET_DOCS_DIR%" -doc-title "%_PROJECT_NAME%" -doc-footer "%_PROJECT_URL%" -doc-version "%_PROJECT_VERSION%" > "%__OPTS_FILE%"
) else (
    echo -classpath "%__CPATH:\=\\%" -siteroot "%_TARGET_DOCS_DIR%" -project "%_PROJECT_NAME%" -project-url "%_PROJECT_URL%" -project-version "%_PROJECT_VERSION%" > "%__OPTS_FILE%"
)
