...
for %%m in (out target) do (
    if %_DEBUG%==1 echo %_DEBUG_LABEL% forfiles /s /p %_ROOT_DIR% /m %%m /c "cmd /c echo @path" 1>&2
    for /f %%i in ('forfiles /s /p %_ROOT_DIR% /m %%m /c "cmd /c if @isdir==TRUE echo @path" 2^>NUL') do (
        if %_DEBUG%==1 echo %_DEBUG_LABEL% rmdir /s /q %%i 1>&2
        rmdir /s /q %%i
        if not !ERRORLEVEL!==0 (
...
if defined __INSTRUMENTED (
    ...
    if exist "!__JACOCO_AGENT_FILE!" (
        ...
        set __RUN_OPTS=-classpath "%_LIBS_CPATH%!__JACOCO_AGENT_FILE!;!__INSTRUMENTED_CLASSES_DIR!"
...
if %_TASTY%==1 (
    ...
    set __RUN_OPTS=-classpath "%_LIBS_CPATH%%_TASTY_CLASSES_DIR%"
