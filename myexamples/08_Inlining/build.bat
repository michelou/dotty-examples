...
if %_LINT%==1 (
    if not defined _SCALAFMT_CMD (
        ...
        set _LINT=0
    ) else if %_SCALA_VERSION%==2 (
        if exist "%_SOURCE_DIR%\main\scala2\" (
            set "_MAIN_SOURCE_DIR=%_SOURCE_DIR%\main\scala2"
        )
    )
)
...
if %_TASTY%==1 if %_SCALA_VERSION%==2 (
...
if %_DEBUG%==1 (
    ...
    echo %_DEBUG_LABEL% Options    : _EXPLAIN=%_SCALAC_OPTS_EXPLAIN% _INSTRUMENTED=%_INSTRUMENTED% _PRINT=%_SCALAC_OPTS_PRINT% _TASTY=%_TASTY% _TIMER=%_TIMER% _VERBOSE=%_VERBOSE% 1>&2
...
if %__N% gtr 1 ( set __N_FILES=%__N% TASTy source files
) else ( set __N_FILES=%__N% TASTy source file
)
if %_DEBUG%==1 ( echo %_DEBUG_LABEL% "%_SCALAC_CMD%" "@%__OPTS_FILE%" "@%__SOURCES_FILE%" 1>&2
...
if "%_VERSION_STRING:NIGHTLY=%"=="%_VERSION_STRING%" (
    ...
    for /f "usebackq" %%i in (`call "%_PWSH_CMD%" -c "$s='%_VERSION_STRING%';$i=$s.indexOf('-bin',0); $s.substring(0, $i)"`) do (
...
@rem )
for /f "delims=" %%i in ('dir /s /b "%_MAIN_SOURCE_DIR%\*.scala" 2^>NUL') do (
...
if %__N% gtr 1 ( set __N_FILES=%__N% Scala test source files
) else ( set __N_FILES=%__N% Scala test source file
)
